...
import { verifyEmailVerificationCode } from '~/server/utils/email-verification-db'
...
export default defineEventHandler(async (event) => {
  ...
  if (!name || !email || password.length < 6 || (verificationEnabled && !code)) {
  ...
  if (role === 'Satıcı') {
    ...
    if (sellerProfile?.categoryIds?.some((categoryId) => !validIds.has(Number(categoryId)))) {
      throw createError({ statusCode: 400, statusMessage: 'Seçilmiş kateqoriyalar yanlışdır' })
    }
  }
