...
export default defineEventHandler(async (event) => {
  ...
  const sellers = users.filter((user: any) => (user.role === 'seller' || user.role === 'Satıcı') && user.status === 'Aktiv')

  const results = sellers.map((seller: any) => {
    const sellerProducts = products.filter((product: any) => Number(product?.sellerId || 0) === Number(seller.id) && product?.status === 'Aktiv')
