...
const applyVarsToRoot = (d: any) => {
  ...
  const deptMenuMaxHeight = Math.max(240, Number(d?.deptMenuMaxHeight || 420))
  const heroSectionBgDesktop = String(d?.heroSectionBgDesktop || primary)
  const themeBgDark = '#020617'
  const surfaceDark = '#0f172a'
  const surfaceSoftDark = '#111827'
  const textDark = '#e5e7eb'
  const textMutedDark = '#cbd5e1'
  ...
  root.style.setProperty('--footer-muted', footerMuted)
  root.style.setProperty('--footer-border', footerBorder)
  root.style.setProperty('--header-nav-bg', headerNavBg)
  root.style.setProperty('--header-nav-link-default', headerNavLinkDefault)
  root.style.setProperty('--header-nav-link-hover', headerNavLinkHover)
  root.style.setProperty('--header-nav-link-active', headerNavLinkActive)
  root.style.setProperty('--dept-btn-bg', deptBtnBg)
  root.style.setProperty('--dept-btn-border', deptBtnBorder)
  root.style.setProperty('--dept-btn-text', deptBtnText)
  root.style.setProperty('--dept-btn-hover-bg', deptBtnHoverBg)
  root.style.setProperty('--dept-btn-hover-border', deptBtnHoverBorder)
  root.style.setProperty('--dept-btn-hover-text', deptBtnHoverText)
  root.style.setProperty('--dept-btn-active-bg', deptBtnActiveBg)
  root.style.setProperty('--dept-btn-active-border', deptBtnActiveBorder)
  root.style.setProperty('--dept-btn-active-text', deptBtnActiveText)
  root.style.setProperty('--dept-menu-width', `${deptMenuWidth}px`)
  root.style.setProperty('--dept-menu-max-h', `${deptMenuMaxHeight}px`)
  root.style.setProperty('--hero-section-bg-desktop', heroSectionBgDesktop)
  root.style.setProperty('--hero-height-mobile', `${mobile}px`)
  root.style.setProperty('--hero-height-desktop', `${desktop}px`)
  root.style.setProperty('--theme-bg', appBg)
  root.style.setProperty('--theme-surface', surfaceBg)
  root.style.setProperty('--theme-surface-soft', '#f8fafc')
  root.style.setProperty('--theme-text', appText)
  ...
  root.style.setProperty('--theme-surface-dark', surfaceDark)
  root.style.setProperty('--theme-surface-soft-dark', surfaceSoftDark)
  root.style.setProperty('--theme-text-dark', textDark)
  root.style.setProperty('--theme-text-muted-dark', textMutedDark)
  root.style.setProperty('--theme-border-dark', borderDark)
}
...
export default defineNuxtPlugin(async () => {
  ...
  watch(
    () => (settingsStore.settings as any).design,
    (d) => applyVarsToRoot(d),
